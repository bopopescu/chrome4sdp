...
void DeprecatedPaintLayer::updateTransform(const ComputedStyle* oldStyle, const ComputedStyle& newStyle)
{
    ...
    if (has3DTransform()) {
        ...
        const double HEAVY_NOTIFICATION_PERIOD = 10;
        // Amount of time to boost, in milliseconds
        const int EXPECTED_HEAVY_DURATION = 10000;
        double now = WTF::currentTime();
        if (now - m_lastHeavyNotification > HEAVY_NOTIFICATION_PERIOD) {
            layoutObject()->document().heavyExecutionDetected(EXPECTED_HEAVY_DURATION);
