...
protected:
    ...
    template <typename MatchFunc, class NodeListType>
    static Element* traverseMatchingElementsForwardToOffset(const NodeListType&, Element& currentElement, const ContainerNode* stayWithin, unsigned offset, unsigned& currentOffset, MatchFunc);
...
template <typename MatchFunc, class NodeListType>
Element* LiveNodeListBase::traverseMatchingElementsForwardToOffset(const NodeListType& nodeList, Element& currentElement, const ContainerNode* stayWithin, unsigned offset, unsigned& currentOffset, MatchFunc isMatch)
{
    ...
    for (Element* next = ElementTraversal::next(currentElement, stayWithin, isMatch); next; next = ElementTraversal::next(*next, stayWithin, isMatch)) {
        if (++currentOffset == offset)
            return next;
        nodeList.saveNode(next, currentOffset);
    }
    return 0;
}
