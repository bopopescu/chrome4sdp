...
Element* HTMLCollection::traverseForwardToOffset(unsigned offset, Element& currentElement, unsigned& currentOffset) const
{
    ...
    switch (type()) {
    case HTMLTagCollectionType:
        return traverseMatchingElementsForwardToOffset(toHTMLTagCollection(*this), currentElement, &rootNode(), offset, currentOffset, makeIsMatch(toHTMLTagCollection(*this)));
    case ClassCollectionType:
        return traverseMatchingElementsForwardToOffset(toClassCollection(*this), currentElement, &rootNode(), offset, currentOffset, makeIsMatch(toClassCollection(*this)));
    default:
        if (overridesItemAfter()) {
            for (Element* next = virtualItemAfter(&currentElement); next; next = virtualItemAfter(next)) {
                if (++currentOffset == offset)
                    return next;
                saveNode(next, currentOffset);
            }
            return nullptr;
        }
        if (shouldOnlyIncludeDirectChildren()) {
        ...
        return traverseMatchingElementsForwardToOffset(*this, currentElement, &rootNode(), offset, currentOffset, makeIsMatch(*this));
    }
}
