...
#  app_manifest_version_code - set the apps version number.
{
  'variables': {
    ...
    'resource_all_zip_path': '<(intermediate_dir)/<(_target_name).resources_all.zip',
    'shared_resources%': 0,
  ...
  'conditions': [
    ['resource_dir!=""', {
      'variables': {
        'resource_input_paths': [ '<!@(find <@(res_extra_dirs) <(resource_dir)  -name "*")' ]
      },
    }],
    ['R_package != ""', {
      'variables': {
  ...
  'actions': [
    {
      ...
      'variables': {
        ...
        'res_input_extra_dirs': ['<@(res_extra_dirs)'],
      ...
      'action': [
        ...
        '--resource-extra-dirs', '<(res_input_extra_dirs)',

        '--resource-zip-out', '<(resource_zip_path)',

        '--all-resources-zip-out', '<(resource_all_zip_path)',
