  ...
  def SetUp(self):
    ...
    if not self.device.HasRoot():
      ...
      if self.device.SetJavaAsserts(self.options.set_asserts):
        ...
        # Check Android release version.  Wait if applicable.
        android_release = self.device.GetProp('ro.build.version.release')
        version = [int(x) for x in android_release.split('.')]
        if version >= [5, 0] and version < [6, 0]:
          # For an Android Lollipop device, sometimes it goes through app
          # optimization before it is fully ready for testing. Device dumpsys
          # activities shows no activity until app optimization completes.
          # So waiting for device acitivity.
          #
          # Preliminary testing on an Android-M-Preview device suggests that
          # app-optimization may not be an issue on Android M devices.
          # TODO: Need to keep track of that.
          pattern = re.compile(r'\s*mFocusedActivity:\s+ActivityRecord'
                               '\{.+\s+(?P<activity>.+?\/.+?)[ \}]')
          output = self.device.RunShellCommand('dumpsys activity activities')
          match = pattern.search('\n'.join(output))
          while match is None:
            # The while-loop is OK because the SetUp is run in a thread when
            # a test runner is created. A watchdog with 7-minute timeout is
            # used to monitor if SetUp completes before the timeout expires.
            # If the timeout does expire, the test run is aborted.
            # TODO: If needed, the timed-out runners could be excluded, and
            # the test run could go ahead with the rest.
            logging.info('Wait for device activity ...')
            time.sleep(5)
            output = self.device.RunShellCommand('dumpsys activity activities')
            match = pattern.search('\n'.join(output))
          logging.info('Found device activity: ' + match.group('activity'))
          # wait for additional seconds to ensure device readiness
          time.sleep(5)
