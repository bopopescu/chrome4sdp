...
#ifdef DO_ZERO_COPY
  ZEROCOPY_LOG("GpuMemoryBufferImplSharedMemory::Create");
#endif
...
#ifdef DO_ZERO_COPY
  ZEROCOPY_LOG("GpuMemoryBufferImplSharedMemory::AllocateForChildProcess");
#endif
  size_t buffer_size = 0u;
  if (!BufferSizeInBytes(size, format, &buffer_size))
...
GpuMemoryBufferImplSharedMemory::CreateFromHandle(
    ...
    const DestructionCallback& callback) {
#ifdef DO_ZERO_COPY
  ZEROCOPY_LOG("GpuMemoryBufferImplSharedMemory::CreateFromHandle");
#endif
