...
void DriverEGL::InitializeExtensionBindings() {
  ...
  ext.b_EGL_QCOM_gpu_perf_hint =
      extensions.find("EGL_QCOM_gpu_perf_hint ") != std::string::npos;
  ...
  debug_fn.eglGpuPerfHintQCOMFn = 0;
  if (ext.b_EGL_QCOM_gpu_perf_hint) {
    fn.eglGpuPerfHintQCOMFn = reinterpret_cast<eglGpuPerfHintQCOMProc>(
        GetGLProcAddress("eglGpuPerfHintQCOM"));
    DCHECK(fn.eglGpuPerfHintQCOMFn);
  }
  ...
  if (ext.b_EGL_KHR_swap_buffers_with_damage) {
    ...
    DCHECK(fn.eglSwapBuffersWithDamageKHRFn);
  }
...
static EGLBoolean GL_BINDING_CALL
Debug_eglGpuPerfHintQCOM(EGLDisplay dpy,
                         EGLContext ctx,
                         const EGLint* attrib_list) {
  GL_SERVICE_LOG("eglGpuPerfHintQCOM"
                 << "(" << dpy << ", " << ctx << ", "
                 << static_cast<const void*>(attrib_list) << ")");
  EGLBoolean result =
      g_driver_egl.debug_fn.eglGpuPerfHintQCOMFn(dpy, ctx, attrib_list);
  GL_SERVICE_LOG("GL_RESULT: " << result);
  return result;
}
...
void DriverEGL::InitializeDebugBindings() {
  ...
  if (!debug_fn.eglGpuPerfHintQCOMFn) {
    debug_fn.eglGpuPerfHintQCOMFn = fn.eglGpuPerfHintQCOMFn;
    fn.eglGpuPerfHintQCOMFn = Debug_eglGpuPerfHintQCOM;
  }
...
EGLBoolean EGLApiBase::eglGpuPerfHintQCOMFn(EGLDisplay dpy,
                                            EGLContext ctx,
                                            const EGLint* attrib_list) {
  return driver_->fn.eglGpuPerfHintQCOMFn(dpy, ctx, attrib_list);
}
...
EGLBoolean TraceEGLApi::eglGpuPerfHintQCOMFn(EGLDisplay dpy,
                                             EGLContext ctx,
                                             const EGLint* attrib_list) {
  TRACE_EVENT_BINARY_EFFICIENT0("gpu", "TraceGLAPI::eglGpuPerfHintQCOM")
  return egl_api_->eglGpuPerfHintQCOMFn(dpy, ctx, attrib_list);
}
