...
typedef EGLBoolean(GL_BINDING_CALL* eglGpuPerfHintQCOMProc)(
    EGLDisplay dpy,
    EGLContext ctx,
    const EGLint* attrib_list);
...
struct ProcsEGL {
  ...
  eglGpuPerfHintQCOMProc eglGpuPerfHintQCOMFn;
...
class GL_EXPORT EGLApi {
 public:
  ...
  virtual EGLBoolean eglGpuPerfHintQCOMFn(EGLDisplay dpy,
                                          EGLContext ctx,
                                          const EGLint* attrib_list) = 0;
...
#define eglGpuPerfHintQCOM ::gfx::g_current_egl_context->eglGpuPerfHintQCOMFn
