...
CHROME_PACKAGE_NAMES = {
  ...
  'swe-chrome':
      ['swe-pkg-name',
       android_browser_backend_settings.SweChromeBackendSettings,
       'SWE_Browser.apk'],
...
def _FindAllPossibleBrowsers(finder_options, android_platform):
  ...
  if (finder_options.browser_executable and
    ...
    package_info = next((info for info in CHROME_PACKAGE_NAMES.itervalues()
                         if info[0] in [exact_package, 'swe-pkg-name']), None)
    if package_info:
      if package_info[0] == 'swe-pkg-name':
        package_info[0] = exact_package
  ...
  for name, package_info in CHROME_PACKAGE_NAMES.iteritems():
    if finder_options.browser_type == name and name == 'swe-chrome':
      assert package_info[0] != 'swe-pkg-name', \
        'For swe-chrome, use --browser-executable=<apk_path>.'
