...
#ifdef DO_ZERO_COPY
class RasterBufferImplBase : public RasterBuffer {
public:
  virtual bool DoZeroCopy() { return false; }
};

class RasterBufferImpl : public RasterBufferImplBase {
#else //DO_ZERO_COPY
...
#ifdef DO_ZERO_COPY
  const Resource* resource() { return resource_; }
#endif
...
#ifdef DO_ZERO_COPY
class RasterBufferZeroImpl : public RasterBufferImplBase {
 public:
  static RasterBufferZeroImpl* Create(ResourceProvider* resource_provider, const Resource* resource) {
    if (resource_provider->HasGpuMemoryBuffer(resource->id())) {
      return new RasterBufferZeroImpl(resource_provider, resource, false);
    }
    if (resource->format() != RGBA_8888 || !swe::PrepareCreateTextureMemory())
      return 0;

    return new RasterBufferZeroImpl(resource_provider, resource, true);
  }

  RasterBufferZeroImpl(ResourceProvider* resource_provider, const Resource* resource, bool should_create)
      : gpu_memory_buffer_write_lock_(resource_provider, resource->id()),
        created_buffer_(false),
        should_create_(should_create),
        resource_(resource) {

  }

  ~RasterBufferZeroImpl() {
    if (should_create_ && !created_buffer_)
      swe::CancelPrepareCreateTextureMemory();
  }

  bool DoZeroCopy() { return true; }
  ...
  void Playback(const RasterSource* raster_source,
                ...
                float scale,
                bool include_images) override {
    gfx::GpuMemoryBuffer* buffer = gpu_memory_buffer_write_lock_.GetGpuMemoryBuffer();
    if (buffer) {
      created_buffer_ = true;
      void* data = 0;
      int stride = 0;
      buffer->Map(&data);
      ...
      TileTaskWorkerPool::PlaybackToMemory(
        ...
        stride, raster_source, raster_full_rect,
        raster_full_rect, scale, include_images);
      buffer->Unmap();
    }
  }
 ...
 private:
  ResourceProvider::ScopedWriteLockGpuMemoryBuffer gpu_memory_buffer_write_lock_;
  bool created_buffer_;
  bool should_create_;
  const Resource* resource_;

  DISALLOW_COPY_AND_ASSIGN(RasterBufferZeroImpl);
};
#endif
...
#ifdef DO_ZERO_COPY
  RasterBufferZeroImpl* buffer = RasterBufferZeroImpl::Create(resource_provider_, resource);
  if (buffer) {
    return make_scoped_ptr<RasterBuffer>(buffer);
  }
#endif
...
#ifdef DO_ZERO_COPY
    RasterBufferImplBase* raster_buffer = static_cast<RasterBufferImplBase*>(task->AsRasterTask()->getRasterBuffer());
    if (raster_buffer && !raster_buffer->DoZeroCopy())
#endif
