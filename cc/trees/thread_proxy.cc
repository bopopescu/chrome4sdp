...
void ThreadProxy::ThrottleRendering(int duration, int slack) {
  throttle_start_time_ = base::Time::Now();
  throttle_duration_ = base::TimeDelta::FromMilliseconds(duration);
  throttle_slack_ = slack;
}
...
void ThreadProxy::ScheduledActionSendBeginMainFrame() {
  ...
  // Default initializer sets delta to zero
  static const base::TimeDelta ZERO_TIME;
  if (throttle_duration_ != ZERO_TIME) {
    if (base::Time::Now() - throttle_start_time_ < throttle_duration_) {
      Proxy::MainThreadTaskRunner()->PostDelayedTask(
          FROM_HERE,
          base::Bind(&ThreadProxy::BeginMainFrame,
                     main_thread_weak_ptr_,
                     base::Passed(&begin_main_frame_state)),
          base::TimeDelta::FromMilliseconds(throttle_slack_));
    }
    else {
      throttle_duration_ = ZERO_TIME;
      Proxy::MainThreadTaskRunner()->PostTask(
          FROM_HERE,
          base::Bind(&ThreadProxy::BeginMainFrame,
                     main_thread_weak_ptr_,
